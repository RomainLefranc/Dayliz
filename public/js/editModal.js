...
const getData = (elem) => {
    ...
    window.axios
        ...
        .then((res) => {
            ...
            heures =
                Math.floor(res.data.data.duree / 3600) > 10
                    ? `${Math.floor(res.data.data.duree / 3600)}`
                    : `0${Math.floor(res.data.data.duree / 3600)}`;
            minutes =
                Math.floor((res.data.data.duree / 60) % 60) > 10
                    ? `${Math.floor((res.data.data.duree / 60) % 60)}`
                    : `0${Math.floor((res.data.data.duree / 60) % 60)}`;
